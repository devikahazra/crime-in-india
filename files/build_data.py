...
CATEGORY_SCHEMA = {
    ...
    'Cyber': [
        ('it_act_offences', 'Offences under IT Act', ['total offences under i.t. act'], []),
        ('ipc_rw_it_offences', 'Offences under IPC r/w IT Act', ['total offences under ipc r/w it act'], []),
        ('sll_rw_it_offences', 'Offences under SLL r/w IT Act', ['total offences under sll r/w it act'], []),
...
# (category, sub_id) pairs needing exact matching even in nested years, where
# the same fix would wrongly affect a same-named sub-indicator in another
# category that doesn't have this collision.
FORCE_EXACT_CATEGORY_IDS = {('IPC', 'grievous_hurt'), ('JuvIPC', 'grievous_hurt')}
...
# NCRB's own source files contain state-name typos/spelling variants that
# don't match the crosswalk's canonical spelling. Found by scanning every
# state marker across all years/sheets (see conversation) -- add new entries
# here if a future year introduces another variant.
STATE_MARKER_ALIASES = {
    'A & N ISLANDS': 'ANDAMAN & NICOBAR ISLANDS',
    'A&N ISLANDS': 'ANDAMAN & NICOBAR ISLANDS',
    'ANDRHA PRADESH': 'ANDHRA PRADESH',
...
def process_year_category(year, category, crosswalk):
    ...
    for idx, row in df.iterrows():
        ...
        if col0.upper().replace(' ', '').startswith('STATE:') or col0.upper().startswith('STATE :'):
            ...
            current_state = STATE_MARKER_ALIASES.get(current_state, current_state)
            continue
    ...
    for sub_id, label, contains_patterns, exact_patterns in schema:
        if fmt == 'simple' or sub_id in FORCE_EXACT_IDS or (category, sub_id) in FORCE_EXACT_CATEGORY_IDS:
